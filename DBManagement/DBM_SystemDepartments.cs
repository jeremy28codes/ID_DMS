...
namespace DMS.DBManagement
{
    public class DBM_SystemDepartments
    {
        ...
        public List<System_departments> ListAll()
        {
            ...
            using (SqlConnection connection = new SqlConnection(sConnectionString))
            {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    list.Add(new System_departments
                    {
        ...
        //GET BY ID
        public System_departments GetBy_ID(int id)
        {
            System_departments item = new System_departments();

            using (SqlConnection connection = new SqlConnection(sConnectionString))
            {
                SqlCommand command = connection.CreateCommand();
                command.CommandType = CommandType.StoredProcedure;
                command.CommandText = "spSystem_departments_GetBy_ID";
        ...
        //CREATE
        public int Insert(System_departments item)
        {
            int id = 0;
            using (SqlConnection connection = new SqlConnection(sConnectionString))
            {
                SqlCommand command = new SqlCommand("spSystem_departments_Insert", connection);
                command.CommandType = CommandType.StoredProcedure;
        ...
        //UPDATE
        public int Update(System_departments item)
        {
            int id = 0;
            using (SqlConnection connection = new SqlConnection(sConnectionString))
            {
                SqlCommand command = new SqlCommand("spSystem_departments_Update", connection);
                command.CommandType = CommandType.StoredProcedure;
        ...
        //DELETE
        public int Delete(System_departments item)
        {
            int id = 0;
            using (SqlConnection connection = new SqlConnection(sConnectionString))
            {
                SqlCommand command = new SqlCommand("spSystem_departments_Delete", connection);
                command.CommandType = CommandType.StoredProcedure;
                command.Parameters.Add("@id", SqlDbType.Int).Value = item.id;
